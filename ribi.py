...
if __name__ == '__main__':
    ...
    користејќи ги првите 85% од даденото податочно множество. Треба да ја пресметате точноста која ја добивате над 
    останатите 15% од податочното множество. 
    Притоа, се користи дел од множеството во кој е отстранета колоната col_index.

    Во почетниот код имате дадено податочно множество. На влез се прима индекс на колоната која треба да се отстрани col_index. 
    ...
    entry = [float(el) for el in input().split(' ')]  #nov zapis
    ...
    classifier = RandomForestClassifier(n_estimators=int(n), criterion=str(crit),random_state=0)

    train_set2 = [] #novo mnozestvo za trening spored col_index
    for t in train_set:
        row = [t[i] for i in range(len(t)) if i != col_index]
        train_set2.append(row)
    train_x2 = []
    for t in train_x:
        row = [t[i] for i in range(len(t)) if i != col_index]
        train_x2.append(row)
    ...
    for t in test_set:
        row = [t[i] for i in range(len(t)) if i != col_index]
        test_set2.append(row)
    test_x2 = []
    for t in test_x:
        row = [t[i] for i in range(len(t)) if i != col_index]
        test_x2.append(row)

    classifier.fit(train_x2,train_y) #treniranje spored podelbata

    acc = 0
    for x,y in zip(test_y, classifier.predict(test_x2)):
        if x == y:
            acc+=1
    acc = acc/ len(test_set2)
    print(f'Accuracy: {acc}')

    entry1 = [entry[i] for i in range(len(entry)) if i != col_index]
    pred_class = classifier.predict([entry1])[0]
    print(pred_class)
    pred_class = classifier.predict_proba([entry1])[0]
    print(pred_class)
